...
class FeatureBuilder:
    ...
    def ensure_continuous_time(df, time_col='datetime'):
        ...
        df = df.ffill().bfill()  # заполняем пропущенные значения в начале и в конце
    ...
    def build_features(self, main_df, trades_df, whale_tx_df):
        ...
        for w in tqdm(self.windows, desc="→ Лог-доходности и Z-Score"):
            ...
            main_df[f'log_return_{w}s'] = main_df[f'log_return_{w}s'].replace([np.inf, -np.inf], np.nan) # обрабатываем бесконечные значения
            main_df[f'price_zscore_{w}s'] = (main_df['price'] - main_df['price'].rolling(w).mean()) / (main_df['price'].rolling(w).std() + 1e-6)
        ...
        for w in tqdm(self.windows, desc="→ Spread & Volume Imbalance Z-Score"):
            window = f'{w}s'
            main_df[f'vol_imbalance_zscore_{w}s'] = (main_df['volume_imbalance'] - main_df['volume_imbalance'].rolling(w).mean()) / (main_df['volume_imbalance'].rolling(w).std() + 1e-6)
            main_df[f'spread_zscore_{w}s'] = (main_df['diff_bid_ask_price'] - main_df['diff_bid_ask_price'].rolling(w).mean()) / (main_df['diff_bid_ask_price'].rolling(w).std() + 1e-6)
        ...
        for w in tqdm(self.windows, desc="→ Фичи из трейдов"):
            window = f'{w}s'
            trade_agg[f'volume_zscore_{w}s'] = (trade_agg['volume'] - trade_agg['volume'].rolling(w).mean()) / (trade_agg['volume'].rolling(w).std() + 1e-6)
            trade_agg[f'imbalance_zscore_{w}s'] = (trade_agg['volume_imbalance'] - trade_agg['volume_imbalance'].rolling(w).mean()) / (trade_agg['volume_imbalance'].rolling(w).std() + 1e-6)
        ...
        for w in tqdm(self.windows, desc="→ Фичи из whale_tx"):
            window = f'{w}s'
            whale_sec[f'whale_tx_zscore_{w}s'] = (whale_sec['whale_tx'] - whale_sec['whale_tx'].rolling(w).mean()) / (whale_sec['whale_tx'].rolling(w).std() + 1e-6)
    ...
    def add_target(self, dataset):
        ...
        # 	•	Значит, имя колонки будет: target_300_50bp - Важно для файла ModelTrainerUpdate
        ...
        # Следующую строку убрали на тестах для оберега от утечки, и добавили 3 следующих
        # dataset[f'target_{max_shift}_{int(self.target_threshold * 10000)}bp'] = labels

        target_column_name = f'target_{max_shift}_{int(self.target_threshold * 10000)}bp'
        dataset[target_column_name] = labels

        # Добавляем NaN в конце, чтобы избежать утечек из будущего
        # dataset[target_column_name].iloc[-max_shift:] = np.nan
        dataset.loc[dataset.index[-max_shift:], target_column_name] = np.nan
